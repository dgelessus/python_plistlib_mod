...
class TestPlutil(unittest.TestCase):
    ...
    def get_lint_status(self):
        return subprocess.check_output(['plutil', "-lint", self.file_name], universal_newlines=True)

    def convert_to_json(self):
        """Convert binary file to json using plutil
        """
        subprocess.check_call(['plutil', "-convert", 'json', self.file_name])

    def convert_to_bin(self):
        """Convert file to binary using plutil
        """
        subprocess.check_call(['plutil', "-convert", 'binary1', self.file_name])
    ...
    def test_lint_status(self):
        ...
        self.assertEqual(self.file_name + ": OK\n", self.get_lint_status())
